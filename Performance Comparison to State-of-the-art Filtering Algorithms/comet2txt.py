...
class scan:
    ...
    def get_qranker_output(self):

        # scan\tcharge\tspectrum neutral mass\tpeptide mass\tdelta_cn\tsp score\txcorr score\tsequence\tmodified sequence\tmodifications\tprotein id\tflanking aa\te-value\n
        common_l=[str(self.scan_number),self.charge,self.measured_mass]  # scan number\
        results = []
        for pep in self.pep_list:
            l=[]
            l.extend(common_l)
            l.append(pep.calculated_mass)
            l.append(pep.deltacn)
            l.append(pep.spscore)
            l.append(pep.xcorr)
            l.append(pep.original_pep)
            l.append(pep.prev_aa+'.'+pep.identified_pep.replace('~','[15.9949]')+'.'+pep.next_aa)
            l.append(pep.modifications)
            l.append(','.join(pep.protein_list))
            l.append(pep.prev_aa+pep.next_aa)
...
def read_one_pep_xml_comet(input_file_str, psm_list, fidx):
            ...
            for search_hit in search_result.iter('{http://regis-web.systemsbiology.net/pepXML}search_hit'):
                ...
                for search_score in search_hit.iter('{http://regis-web.systemsbiology.net/pepXML}search_score'):
                    if search_score.attrib['name']=='xcorr':
                        pep.xcorr = search_score.attrib['value']
                    if search_score.attrib['name']=='deltacn':
                        pep.deltacn = search_score.attrib['value']
                    if search_score.attrib['name']=='spscore':
                        pep.spscore= search_score.attrib['value']
                    if search_score.attrib['name']=='expect':
                        pep.evalue= search_score.attrib['value']
                # get proteins
...
def write_output(psm_list, output_file_str):
    with open(output_file_str, 'w') as fw:
        header_str = 'scan\tcharge\tspectrum neutral mass\tpeptide mass\tdelta_cn\tsp score\txcorr score\tsequence\tmodified sequence\tmodifications\tprotein id\tflanking aa\te-value'
        fw.write(header_str)
        fw.write('\n')
        for v in psm_list:
            if v.pep_list:
                ll = v.get_qranker_output()
                for l in ll:
                    fw.write('\t'.join(l))
                    fw.write('\n')
...
def main():
    ...
    if len(opts)==0:
        print("\n\nUsage:\n")
        print("-i\t Comet's xml format output file for conversion")
    ...
    for opt, arg in opts:
        if opt in ("-h"):
            ...
            output_file=arg
    psm_list = []
    prefix=input_file.split('.')[1].split('_')[-1]
    read_one_pep_xml_comet(input_file,psm_list,prefix)
