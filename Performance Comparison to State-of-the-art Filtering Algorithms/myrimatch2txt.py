...
class scan:
    ...
    def get_qranker_output(self):
        self.find_diff()
        # scan\tcharge\tspectrum neutral mass\tpeptide mass\tmvh_diff\tmz score\txcorr score\tsequence\tmodified sequence\tmodifications\tprotein id\tflanking aa\n
        common_l=[str(self.scan_number),self.charge,self.measured_mass]
        results = []
...
def write_output(psm_list, output_file_str):
    with open(output_file_str, 'w') as fw:
        header_str = 'scan\tcharge\tspectrum neutral mass\tpeptide mass\tmvh_diff\tmz score\tmvh score\tsequence\tmodified sequence\tmodifications\tprotein id\tflanking aa'
        fw.write(header_str)
        fw.write('\n')
        for v in psm_list:
            if v.pep_list:
