...
class scan:
    ...
    def find_diff(self):
        pep_sorted_list = sorted(self.pep_list, key=lambda pep: -pep.EValue)
        for i in range(len(pep_sorted_list) - 1):
            pep_sorted_list[i].score_diff = pep_sorted_list[i].EValue - pep_sorted_list[i + 1].EValue

    def get_qranker_output(self):
        self.find_diff()
        # scan\tcharge\tspectrum neutral mass\tpeptide mass\tscore_diff\tEValue\tsequence\tmodified sequence\tmodifications\tprotein id\tflanking aa\n
        results = []
        for pep in self.pep_list:
            l = [str(self.scan_number), pep.charge, str(self.measured_mass)]
            l.append(str(pep.calculated_mass))
            l.append(str(pep.score_diff))
...
def write_output(psm_list, output_file_str):
    with open(output_file_str, 'w') as fw:
        header_str = 'scan\tcharge\tspectrum neutral mass\tpeptide mass\tscore_diff\tEValue\tsequence\tmodified sequence\tmodifications\tprotein id\tflanking aa'
        fw.write(header_str)
        fw.write('\n')
        for v in psm_list:
            if v.pep_list:
